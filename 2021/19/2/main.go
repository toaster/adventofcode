...
func main() {
	...
	for i, a := range final {
		for j, b := range final {
			s := a.position.Subtract(b.position)
			dist := math.AbsInt(s.X) + math.AbsInt(s.Y) + math.AbsInt(s.Z)
			fmt.Printf("%d, %d: %v - %v = %v (%5d)\n", i, j, a.position, b.position, s, dist)
			distances = append(distances, dist)
		}
	}
	sort.Ints(distances)
	fmt.Println(distances[len(distances)-1])
